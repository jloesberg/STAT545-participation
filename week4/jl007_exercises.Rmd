---
title: "jl007 Exercises: Practice with `dplyr`"
output: 
  html_document:
    keep_md: true
    theme: paper
...
gapminder %>% 
  ...
  arrange(min_life)
#summ creating new column and using it later, so didnt need mutate. [] is defining within country
```
...
gapminder %>% 
  group_by(country) %>% 
  arrange(year) %>% #default is by ascending order
...
gapminder %>% 
  group_by(country) %>% 
  arrange(year) %>%
  mutate(inc_life_exp = difference(lifeExp)) %>% #difference fucntion is finding differnce between one row and the one before it, so like the lag() funtion..is NA for all 1952s then
  drop_na() %>% #getting rid of all 1952 values
  ungroup() %>% #ungrouping what we grouped
  group_by(continent) %>% 
