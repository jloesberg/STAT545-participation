---
title: "jl008 Exercises: Fix the Plots"
output: 
  html_document:
    keep_md: true
    theme: paper
...
ggplot(mpg, aes(cty, hwy)) +
  geom_jitter(alpha = 0.5) + #changes scale for points (heavier pts = more)
  geom_smooth(method = "lm")+ #adding trendline, its using the original aes from the first command
  theme_bw() #changing grey background
```

![](jl_008_files/figure-html/unnamed-chunk-2-1.png)<!-- -->
...
*it doesnt like it when you group by for dplyr, need to do it in ggplot*


```r
gapminder %>% 
  # group_by(country) %>% get rid of!!
  ggplot(aes(year, lifeExp, group = country, color = country == "Rwanda")) + #just coloring one country
  geom_line(alpha = 1/3) + #adding transparency
  scale_color_discrete("", labels = c("Other", "Rwanda")) #getting rid of title of legend and chaning keys
```

![](jl_008_files/figure-html/unnamed-chunk-3-1.png)<!-- -->
...
ggplot(gapminder, aes(gdpPercap, lifeExp)) +
  ...
  scale_x_log10(labels = scales::comma_format()) +#chaning names of x axis
  facet_wrap(~ continent) #splits plot up into individual per each continent...shares axis...wow so cool! scales = "free" lets each panel have own axes...scales = "free_y" or x frees just one
```

![](jl_008_files/figure-html/unnamed-chunk-4-1.png)<!-- -->
...
```r
gapminder %>% 
  filter(continent != "Oceania") %>% #get rid of oceania
  ggplot(aes(gdpPercap, lifeExp, size = pop, color = continent)) + #can add size = pop
  facet_wrap(~ continent, nrow = 1) + #can add nrow = 1, makes it skinnier
  geom_point(alpha = 0.5, shape = 21) + #shapes = 21 makes them holow circles
  scale_x_log10(labels = scales::comma_format()) +
  scale_size_area()
```

![](jl_008_files/figure-html/unnamed-chunk-5-1.png)<!-- -->

```r
#what func to add options? ggplot or geom? if ggplot, applies to all
```
...
![](jl_008_files/figure-html/unnamed-chunk-6-1.png)<!-- -->
...
![](jl_008_files/figure-html/unnamed-chunk-7-1.png)<!-- -->
...
![](jl_008_files/figure-html/unnamed-chunk-8-1.png)<!-- -->
...
![](jl_008_files/figure-html/unnamed-chunk-9-1.png)<!-- -->
...
![](jl_008_files/figure-html/unnamed-chunk-10-1.png)<!-- -->
...
![](jl_008_files/figure-html/unnamed-chunk-12-1.png)<!-- -->
...
![](jl_008_files/figure-html/unnamed-chunk-14-1.png)<!-- -->
